...
const socialItems: SocialItem[] = [
  ...
  },
  {
    ...
    icon: <BrandDiscordIcon className="size-4 text-white" />,
  ...
  },
  {
    ...
    icon: <BrandTelegramIcon className="size-4.5 text-white" />,
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4">
        {socialItems.map((item, idx) => (
          <a
            ...
            className={`group flex items-center justify-between px-4 sm:px-5 py-4 sm:py-4.5 hover:bg-zinc-900/50 transition-colors border-b border-edge ${
              idx % 2 === 0 ? "border-r border-edge" : "border-r-0"
            } ${idx % 4 !== 3 ? "sm:border-r sm:border-edge" : "sm:border-r-0"}`}
          >
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={`flex size-8.5 sm:size-9 shrink-0 items-center justify-center rounded-lg ${item.containerClass}`}
              >
                {item.icon}
              </div>
              <span className="text-sm sm:text-[14.5px] font-medium text-zinc-200 group-hover:text-white transition-colors truncate">
                {item.name}
              </span>
            </div>
            <ArrowUpRight className="size-4 text-zinc-600 group-hover:text-zinc-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-150 shrink-0 ml-1.5" />
          </a>
        ))}
      </div>
    </section>
